...
const RetenoPluginFunctions = {
  ...
  logEcommerceEvent: function (arg0, success, error) {
    ...
    if (
      ...
      payload.eventType.trim().length === 0
    ) {
  ...
  setAnonymousUserAttributes: function (arg0, success, error) {
    ...
    if (payload.userAttributes && (payload.userAttributes.phone || payload.userAttributes.email)) {
      return Promise.reject(
        new Error(
          'Anonymous user attributes cannot include phone/email. Use setUserAttributes(externalUserId, user) instead.'
        )
  ...
  setInAppMessagesPauseBehaviour: function (arg0, success, error) {
    if (!arg0 || typeof arg0 !== 'string') {
      return Promise.reject(
        new Error("Missing argument: behaviour ('SKIP_IN_APPS' or 'POSTPONE_IN_APPS')")
      );
    }
  ...
  /*
        ...
        Set showSummary: true to let the plugin maintain a group-summary notification.
  ...
  setNotificationGroupingRule: function (arg0, success, error) {
    ...
    if (config.showSummary !== undefined && typeof config.showSummary !== 'boolean') {
      return Promise.reject(new Error('Invalid argument: showSummary must be a boolean'));
    }
    ...
    if (config.showSummary === true) {
      normalizedConfig.showSummary = true;
    }
